...
\begin{enumerate}
	...
	\item Choose whether to add no more terms (\texttt{nnlo-ct}); add the log squared  terms  (\texttt{nnlo-logSq\_ct});  add the log-squared terms,  the single log terms, and the semi-NNLO pure sunset term \emph{without} Bijnen's central values for the $L_i$'s  (\texttt{nnlo-full}); or add the log-squared terms,  the single log terms, and the semi-NNLO pure sunset term \emph{with} Bijnen's central values for the $L_i$'s  (\texttt{nnlo-full\_bijnens}).
\end{enumerate}
