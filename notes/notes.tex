...
\begin{figure}
	...
	\caption{Extrapolations of $F^\pm_K/F^\pm_\pi$ to the physical point for the various models, with the FLAG result shown as a red band. Without a better selection of priors for the NNLO and NNNLO LECs, the NNLO and NNNLO fits cannot be tightly determined.}
\end{figure}

\begin{figure}
	\includegraphics[width=0.9\textwidth]{./../tmp/delta_su2_comparison.pdf}
...
\foreach \model in {ma, ma-taylor, xpt, xpt-taylor}
{
	...
	\foreach \order in {nlo, nnlo, nnnlo}
	{
		\begin{figure}[H]
			\caption{\model (\order)}
			\includegraphics[width=\textwidth, page=3]{"./../tmp/\model"_"\order".pdf}
		\end{figure}
	}
}
