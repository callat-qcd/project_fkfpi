...
The decomposition of the log-squared terms and the sunset terms is not unique. The remaining terms two-loop terms are encapsulated by the following function, which is implemented in \texttt{CHIRON} \cite{Bijnens:2014gsa}. A python wrapper of \texttt{chiron.FF} was written for this project.
\begin{equation}
...
In our analysis, we have three choices for the chiral symmetry scale:
\begin{equation}
\Lambda_\chi  = 4 \pi \{ F_\pi, \sqrt{F_\pi F_K}, F_K\} \, .
\end{equation}
We make the agnostic choice of averaging the results of all three choices (see section \ref{section:model_average}). However, there's a nick in this approach: the choice of $F$ actually introduces errors at NNLO.
