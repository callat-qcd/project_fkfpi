// jbdtheta3.cc is part of the numerical library jbnumlib
...
double jbdtheta3(const double u,const double q)
  /*
     computes 1+2\sum_{n=1,\infty} q^(n^2)cos(2n\pi u)

     for q<= 0.5 simply sums the above series, up to n=10 terms

     for q >= 0.5 uses the inverse formula from
        
        theta3(u,q) = sqrt(pi/|lnq|)*exp(-pi^2 u^2/|lnq|)
	*theta3(-iu pi/|lnq|,exp(-pi^2/|lnq|))

     precision works almost to machine precision (tested by comparing it
     against running with much higher nmaxu and nmaxl and comparing the two
  ...
  */
{
  ...
  if( q < 0.5 ){
    ...
    for (int i=nmaxl; i>0;i--){
      result += a[i]*cos(2.*M_PI*u*double(i));
    }
    return 1.+2.*result;
  }
  // case >= 0.5
  ...
  // everything put in the exponentials otherwise overflow
    ...
    return sqrt(lam1)*(exp(-M_PI*u*u*lam1)+2.*result);// first term is n=0
}
