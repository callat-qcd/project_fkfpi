// jbdtheta32.cc is part of the numerical library jbnumlib
...
double jbdtheta32(const double q)
  /*
     computes 1+2\sum_{n=1,\infty} n^2  q^(n^2)

     for q<= 0.5 simply sums the above series, up to n=10 terms

     for q >= 0.5 uses the derivative of
  ...
  */
{
  ...
  for( int i = 0; i<= nmaxu; i++){
    a[i] = atemp*(1./(2.*M_PI)-lam1*double(i*i));
    atemp *= p;
    p *= lamsq;
  }
  double result = 0.;
  ...
  return sqrt(lam1)*lam1*(1./(2.*M_PI)+2.*result);
}
